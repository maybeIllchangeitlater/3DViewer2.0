...
namespace s21{
    ...
    public:
        ...
        virtual ~TransformationStrategy() = default;
         virtual void ApplyTranslation(QMatrix4x4& matrix, QVector<float>& vertex,float x, float y, float z) const = 0;
         virtual void ApplyScaling(QMatrix4x4& matrix, QVector<float>& vertex, float scale) const = 0;
         virtual void ApplyRotation(QMatrix4x4& matrix, QVector<float>& vertex, float x, float y, float z) const = 0;
         virtual void MoveModel(QMatrix4x4& matrix, QVector<float>& vertex, const Settings& settings) const = 0;
        /**
