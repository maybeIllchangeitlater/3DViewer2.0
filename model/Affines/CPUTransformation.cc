...
void CPUTransformation::ApplyTranslation(QMatrix4x4 &matrix, QVector<float> &vertex, float x, float y, float z) const
{
    Q_UNUSED(matrix);
...
void CPUTransformation::ApplyScaling(QMatrix4x4 &matrix, QVector<float> &vertex, float scale) const
{
    Q_UNUSED(matrix);
    for(auto& v : vertex)
        v *= scale;
...
void CPUTransformation::ApplyRotation(QMatrix4x4 &matrix, QVector<float> &vertex, float x, float y, float z) const
{
    Q_UNUSED(matrix);
    std::swap(x, y);
    y/=60;
...
void CPUTransformation::MoveModel(QMatrix4x4 &matrix, QVector<float> &vertex, const Settings &settings) const
{
    Q_UNUSED(matrix);
